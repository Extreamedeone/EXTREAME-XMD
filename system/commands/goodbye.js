...
async function handleGoodbye(sock, chatId, message, match) {
    ...
    if (lower === 'on') {
        if (await isGoodByeOn(chatId)) {
            return sock.sendMessage(chatId, { text: '⚠️ Goodbye messages are *already enabled*.', ...channelInfo, quoted: message });
        }
        await addGoodbye(chatId, true, null);
        return sock.sendMessage(chatId, { text: '✅ Goodbye messages *enabled*. Use *.goodbye [your message]* to customize.', ...channelInfo, quoted: message });
    }

    if (lower === 'off') {
        if (!(await isGoodByeOn(chatId))) {
            return sock.sendMessage(chatId, { text: '⚠️ Goodbye messages are *already disabled*.', ...channelInfo, quoted: message });
        }
        await delGoodBye(chatId);
        return sock.sendMessage(chatId, { text: '✅ Goodbye messages *disabled* for this group.', ...channelInfo, quoted: message });
    }


    await delay(2000);
    await addGoodbye(chatId, true, match);
    return sock.sendMessage(chatId, { text: '✅ Custom goodbye message *set successfully*.', ...channelInfo, quoted: message });
}

async function goodbyeCommand(ben, chatId, m, match) {
const sock = ben;
const message = m;
